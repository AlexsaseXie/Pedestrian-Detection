...
from src.new_dataset import NewDataset
...
def train(opt):
    ...
    training_set = NewDataset(opt.data_path, opt.image_size)
    ...
    criterion = NewYoloLoss(model.anchors, opt.reduction)
    ...
    for epoch in range(opt.num_epoches):
        ...
        for iter, batch in enumerate(training_generator):
            ...
            optimizer.zero_grad()
            logits = model(image)
            loss, loss_coord, loss_conf = criterion(logits, label)
            loss.backward()
            optimizer.step()
            print("Epoch: {}/{}, Iteration: {}/{}, Lr: {}, Loss:{:.2f} (Coord:{:.2f} Conf:{:.2f} )".format(
                epoch + 1,
                opt.num_epoches,
                iter + 1,
                num_iter_per_epoch,
        ...
        if epoch % opt.test_interval == 0:
            ...
            print("Epoch: {}/{}, Lr: {}, Loss:{:.2f} (Coord:{:.2f} Conf:{:.2f} )".format(
                epoch + 1,
                opt.num_epoches,
                optimizer.param_groups[0]['lr'],
                te_loss,
